    ...
    class Semaphore
      ...
      class FiberNode
        ...
        def initialize(fiber, blocker)
          ...
          @use_transfer = scheduler.respond_to?(:transfer) && scheduler.respond_to?(:resume)
          @waiter = {
            fiber: fiber,
            scheduler: scheduler,
            ready: false,
        ...
        def suspend
          @use_transfer ? Fiber.scheduler.transfer : Fiber.scheduler.block(@blocker, nil)
        end
        ...
        def resume
          ...
          scheduler = Fiber.scheduler
          @use_transfer ? scheduler.resume(fiber) : scheduler.unblock(@blocker, fiber)
        end
      end
